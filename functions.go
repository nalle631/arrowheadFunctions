...
func PublishService(requestBody Service) {

	payload, err := json.Marshal(requestBody)
	if err != nil {
		log.Fatal(err)
	}
	req, err := http.NewRequest("POST", "https://localhost:8443/serviceregistry/register", bytes.NewBuffer(payload))
	if err != nil {
		log.Fatal(err)
	}
...
func RemoveService(service Service) {
	...
	url := fmt.Sprintf("https://localhost:8443/serviceregistry/unregister?address=%s&port=%s&service_definition=%s&service_uri=%s&system_name=%s", service.ProviderSystem.Address, strconv.Itoa(service.ProviderSystem.Port), service.ServiceDefinition, service.ServiceUri, service.ProviderSystem.SystemName)
